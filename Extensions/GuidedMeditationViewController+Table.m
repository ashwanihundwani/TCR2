//
//  GuidedMeditationViewController+Table.m
//  TinnitusCoach
//
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0)
    {
        ...
        info.descriptionText = @"Guided meditation exercises take the guesswork out of relaxing.  All you have to do is follow the directions on the recording.";
        
        info.skillImage = [UIImage imageNamed:@"4GuidedMeditation.png"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 2)
    {
        switch (indexPath.row) {
            case 0:
            {
                [self PlayAudioOne:self];
            }
                break;
            case 1:
            {
                [self PlayAudioTwo:self];
            }
                break;
                
            case 2:
            {
                [self PlayAudioThree:self];
            }
                break;
                
            case 3:
            {
                [self PlayAudioFour:self];
            }
                break;
                
            case 4:
            {
                [self PlayAudioFive:self];
            }
                break;
